    ...
    public void ActivityMenu()
    {
        ...
        switch (input)
        {
            case "1":
                ...
                break;
            case "2":
                ViewAllActivities(_currentUser);
                break;
            case "3":
         ...
         void ListActivitiesByType()
        {
            ...
            {
                var selectedActivities = fitnessAppContext.SportActivities.Where(a => (int)a.ActivityType == selectedType).ToList();
         ...
         void ListActivitiesByDate()
        {
            ...
            {
                var selectedActivities = fitnessAppContext.SportActivities.Where(a => a.ActivityDate.Date == selectedDate.Date).ToList();
                if (selectedActivities.Any())
                {
         ...
         void ViewAllActivities(User currentUser)
        {
            ...
            var userActivities = fitnessAppContext.SportActivities;
         ...
         void UpdateActivityDetails()
        {
            ...
            {
                var activityToUpdate = fitnessAppContext.SportActivities.FirstOrDefault(a => a.Id == new Guid(activityId.ToString()));
        ...
        void DeleteActivityById()
        {
            ...
            if (Guid.TryParse(activityIdInput, out Guid activityId))
            {
                var activityToDelete = fitnessAppContext.SportActivities.FirstOrDefault(a => a.Id == activityId);

                if (activityToDelete != null)
                {
                    fitnessAppContext.SportActivities.Remove(activityToDelete);
                    fitnessAppContext.SaveChanges();
        ...
        void RemoveAllActivities()
        {
            ...
            if (confirmation?.ToUpper() == "Y" || confirmation?.ToUpper() == "YES")
            {
                try
                {
                    ...
                    fitnessAppContext.SportActivities.RemoveRange(fitnessAppContext.SportActivities);
                    fitnessAppContext.SaveChanges();
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"Error: {ex.Message}");
                    Console.ResetColor();
                    Console.WriteLine("Failed to delete all activities.");
                }
            }
            else
            {
